package com.ttcode.api.b.factory;


import com.ttcode.api.b.RemoteApplicationBService;
 ...
 */
@Component
public class RemoteApplicationBFactory implements FallbackFactory<RemoteApplicationBService> {
    Logger log = LoggerFactory.getLogger(RemoteApplicationBFactory.class);

    @Override
    public RemoteApplicationBService create(Throwable cause) {
        log.info("application a invoke error: {}", cause.getMessage());
        return new RemoteApplicationBService() {
            @Override
